...
- `generate_model(llm, prompt, model_cls, fallback)` — asks for JSON, validates it into a **Pydantic model**, and on failure does a **corrective retry** (feeds the bad output, the error, and the target schema back to the model to fix — not a blind re-prompt), then falls back to a deterministic object (the reference pattern for future LLM-based parsers).
