...
- `generate_model(llm, prompt, model_cls, fallback)` — asks for JSON, validates it into a **Pydantic model**, **retries once** on a bad/unparseable response (LLMs often self-correct), then falls back to a deterministic object (the reference pattern for future LLM-based parsers).
