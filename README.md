...
- `generate_model(llm, prompt, model_cls, fallback)` — asks for JSON, validates it into a **Pydantic model**, and falls back to a deterministic object on any failure (the reference pattern for future LLM-based parsers).
